...
public class ContentWriter {
    ...
    public static boolean writeContent(String outFile, HashMap<String, HashMap<String, Double>> resultContainer) {
        ...
        boolean written = false;
        try {
            FileWriter fwriter = new FileWriter(new File(outFile), false);
    ...
    public static boolean writeContentForSTscore(String outFile, HashMap<String, HashMap<String, Double>> resultContainer) {
        // TODO Auto-generated method stub
        //System.out.println(resultContainer);
        boolean written = false;
        try {
            ...
            for (String key:resultContainer.keySet()) {
                //fwriter.write(key+ "\n");
                HashMap<String, Double> queryContent=resultContainer.get(key);
                if(queryContent.size()==0)fwriter.write(key+":"+ queryContent.size() +":0"+"\n");
               // else if(queryContent.size()>0)fwriter.write(key+":");
                for(String key2:queryContent.keySet())
                {
                    fwriter.write(key+":"+key2+":"+queryContent.get(key2)+"\n");
                }
            }
            fwriter.close();
            written = true;
